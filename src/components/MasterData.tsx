...
export const MasterData: React.FC = () => {
  ...
  const handleSaveSchool = (e: React.FormEvent) => {
    ...
    setTimeout(() => setSchoolSavedNotice(false), 3000);
  };
      ...
      babs: [
        {
          id: `bab-${Date.now()}-1`,
          nama: "BAB I",
          judul: "Materi Awal Bab I",
          tps: [
            {
              id: `tp-${Date.now()}-1`,
              kode: "TP 1",
              deskripsi: "Tujuan Pembelajaran 1",
              babId: `bab-${Date.now()}-1`
            }
          ]
        }
      ]
  ...
  return (
    <div className="space-y-6">
      {/* Multi-Tenant Guru & Supabase Cloud Status Card */}
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-xl border border-indigo-900/50 p-4 text-white shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-start gap-3">
            <div className="p-2.5 rounded-lg bg-indigo-600/30 border border-indigo-400/30 text-indigo-300">
              <Database className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-sm text-white">Isolasi Data Multi-Tenant Guru (Supabase)</span>
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-mono bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                  <ShieldCheck className="w-3 h-3" /> auth.uid() Aktif
                </span>
              </div>
              <p className="text-xs text-indigo-200/80 mt-0.5">
                Akun: <span className="font-mono text-white font-medium">{user?.email || "Guru Kelas"}</span> • ID: <span className="font-mono text-indigo-300 text-[11px]">{user?.id?.slice(0, 16)}...</span>
              </p>
              <p className="text-[11px] text-slate-400 mt-1">
                Tabel Terintegrasi: <span className="text-slate-300 font-mono">sipena_config</span>, <span className="text-slate-300 font-mono">sipena_siswa</span>, <span className="text-slate-300 font-mono">sipena_mapel</span>, <span className="text-slate-300 font-mono">sipena_nilai_formatif</span>, <span className="text-slate-300 font-mono">sipena_nilai_sumatif</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-end sm:self-center">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600/80 hover:bg-indigo-600 border border-indigo-400/40 text-xs font-medium text-white transition cursor-pointer"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${syncStatus === 'syncing' ? 'animate-spin' : ''}`} />
              <span>{syncStatus === 'syncing' ? 'Menyinkronkan...' : 'Sinkronkan Cloud'}</span>
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-6">
          ...
          <form onSubmit={handleSaveSchool} className="space-y-6">
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end">
              <button
                type="submit"
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-semibold shadow-xs transition-colors"
              >
