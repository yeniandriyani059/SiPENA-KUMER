...
export const GradeInput: React.FC = () => {
  ...
  // Auto fill demo values for students with empty grades
  const handleFillDemoValues = () => {
    filteredStudents.forEach((student, idx) => {
      const base = 70 + ((idx * 7) % 25);
      allTps.forEach((tp, tpIdx) => {
        const score = Math.min(100, Math.max(50, base + ((tpIdx % 3) - 1) * 2));
        updateStudentGradeField(student.id, currentSubject.id, selectedSemester, "formatif", "tpScores", score, tp.id);
      });
      updateStudentGradeField(student.id, currentSubject.id, selectedSemester, "formatif", "ulanganHarian", base);
      updateStudentGradeField(student.id, currentSubject.id, selectedSemester, "formatif", "tugasRutin", base + 2);
      updateStudentGradeField(student.id, currentSubject.id, selectedSemester, "formatif", "praktikProyek", base + 1);

      currentSubject.babs.forEach((bab, bIdx) => {
        const bScore = Math.min(100, Math.max(50, base + (bIdx % 2 === 0 ? 3 : -2)));
        updateStudentGradeField(student.id, currentSubject.id, selectedSemester, "sumatif", "babScores", bScore, bab.id);
      });

      updateStudentGradeField(student.id, currentSubject.id, selectedSemester, "sumatif", "astsNonTes", base + 2);
      updateStudentGradeField(student.id, currentSubject.id, selectedSemester, "sumatif", "astsTes", base - 1);
      updateStudentGradeField(student.id, currentSubject.id, selectedSemester, "sumatif", "asasNonTes", base + 1);
      updateStudentGradeField(student.id, currentSubject.id, selectedSemester, "sumatif", "asasTes", base);
    });
  };
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="Isi otomatis nilai sampel bagi siswa yang belum lengkap"
              className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 rounded-lg font-medium transition-colors"
            >
              <Sparkles className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Isi Sampel</span>
            </button>

            <button
              onClick={() => setActiveTab("cetak-mapel")}
              className="inline-flex items-center gap-1 px-3 py-1.5 text-xs bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-semibold shadow-xs transition-colors"
            >
